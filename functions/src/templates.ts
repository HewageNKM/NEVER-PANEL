    ...
    switch (paymentStatus) {
        case "Pending":
            if (paymentMethod === "COD") {
                message += `
                            Your order (ID: ${orderId}) has been 
                            confirmed for COD.
        ...
        case "Failed":
            if (paymentMethod === "COD") {
                message += `
                            Your COD order 
                            (ID: ${orderId}) has failed.
                            Amount: LKR ${totalAmount}
                            Please try placing 
                            your order again.`;
                                } else if (paymentMethod === "PayHere") {
                                    message += `
                            Your order (ID: ${orderId}) has failed 
                            to process via PayHere.
                            Amount: LKR ${totalAmount}
        ...
        default:
            message += `Your order (ID: ${orderId}) 
                        status is currently ${paymentStatus}.
                        
                        `;
            break;
    }

    message += `NEVERBE Team`;
...
const orderStatusUpdateSMS = (
    ...
    switch (status) {
        ...
        case "Cancelled":
            ...
            that your order (ID: ${orderId}) has 
            been cancelled.
